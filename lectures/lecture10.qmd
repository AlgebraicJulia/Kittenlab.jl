...
## A new perspective on coproducts in Julia

We can do the coproduct of two types in Julia like the following
...
Now, suppose we want to write a function `foo` out of `Coproduct{Int,String}` into `Int` that takes `Left(x)` to `x^2` `Right(x)` to `length(x)`. We can use Julia's dispatch system to help us here:
...
We're not going to go over pushouts again in this lecture; we'll do that next lecture. But now we know how we are going to characterize the pushout of a diagram in a category $\C$: we are going to describe what the maps out of the pushout object look like by writing down a certain $\C$-set, and then we are going to find a representative of that $\C$-set!
